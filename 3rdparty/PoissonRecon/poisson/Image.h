...
struct ImageReader {
    ...
    static unsigned char* ReadColor(const char* fileName,
        ...
        for (unsigned int j = 0; j < height; j++) {
            ...
            if (channels == 3)
                memcpy(pixels + j * width * 3, pixelRow, sizeof(unsigned char) * width * 3);
            else if (channels == 1)
...
struct ImageWriter {
    ...
    virtual unsigned int nextRows(const unsigned char* rows, unsigned int rowNum) {
        unsigned int row;
        for (unsigned int r = 0; r < rowNum; r++) row = nextRow(rows + _width * _channels * r);
        return row;
    }
...
protected:
    ...
    char** _tileNames;
    unsigned int _tileRows, _tileColumns, _currentPixelRow, _currentTileRow, *_tileWidths,
            *_tileHeights;
};
...
bool TiledImageReader::GetInfo(const char* fileName,
    ...
    }
    {
        ...
        if (sscanf(line, "Columns: %d", &_tileColumns) != 1)
            ERROR_OUT("Failed to read column count from: ", fileName, " (", line, ")");
        if (!fgets(line, 1024, fp)) ERROR_OUT("Failed to read row line from: ", fileName);
        line[strlen(line) - 1] = 0;
        if (sscanf(line, "Rows: %d", &_tileRows) != 1)
            ERROR_OUT("Failed to read row count from: ", fileName, " (", line, ")");
        ...
        for (unsigned int r = 0; r < _tileRows; r++)
            for (unsigned int c = 0; c < _tileColumns; c++) {
                ...
                if (!r && !c)
                    ...
                    ERROR_OUT("Number of color channels don't match: ", _channels, " != ", _c);
                if (!r)
                    _tileWidths[c + 1] = _w;
                else if (_tileWidths[c + 1] != _w)
                    ERROR_OUT("Images in the same column must have the same width: ",
                              _tileWidths[c + 1], " != ", _w);
                if (!c)
                    _tileHeights[r + 1] = _h;
                else if (_tileHeights[r + 1] != _h)
                    ERROR_OUT("Images in the same row must have the same heights: ",
                              _tileHeights[r + 1], " != ", _h);
            }
    }
    fclose(fp);
...
TiledImageReader::TiledImageReader(const char* fileName,
    ...
    for (unsigned int c = 0; c < _tileColumns; c++) _tileWidths[c + 1] += _tileWidths[c];
    for (unsigned int r = 0; r < _tileRows; r++) _tileHeights[r + 1] += _tileHeights[r];
    width = _width = _tileWidths[_tileColumns], height = _height = _tileHeights[_tileRows],
    channels = _channels;
...
unsigned TiledImageReader::nextRow(unsigned char* row) {
    ...
    if (_currentPixelRow == _tileHeights[_currentTileRow])
        for (unsigned int c = 0; c < _tileColumns; c++)
            _tileReaders[c] = ImageReader::Get(_tileNames[_currentTileRow * _tileColumns + c]);
...
TiledImageWriter::TiledImageWriter(const char* fileName,
                                   ...
                                   unsigned int channels,
                                   ImageWriterParams params) {
    _width = width, _height = height, _channels = channels, _tileWidth = params.tileWidth,
...
unsigned int TiledImageWriter::nextRow(const unsigned char* row) {
    ...
    if ((_currentPixelRow % _tileHeight) == 0) {
        for (unsigned int c = 0; c < _tileColumns; c++)
            _tileWriters[c] =
                    ImageWriter::Get(_tileNames[r * _tileColumns + c],
    ...
    if ((_currentPixelRow % _tileHeight) == (_tileHeight - 1) || _currentPixelRow == (_height - 1))
